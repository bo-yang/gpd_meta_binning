...
my $clustedreads=keys %$clusts; # number of clustered reads
...
while (my $line = <IN>) {
	chomp $line;
	if($line =~ /^>/) {
		(my $readID,my $loc,my $gi, my $gi_num, my $ref, my $ref_id, my $source_1)=split(/\|/,$line);
		$source_1 =~ s/ /_/g;
		...
		if (not $clustcladestats->{$clusts->{$readID}}->{$spec}) {
			$clustcladestats->{$clusts->{$readID}}->{$spec}=1;
		} else {
			$clustcladestats->{$clusts->{$readID}}->{$spec}++;
		}
		...
		if (not $clustcladestats->{$clusts->{$readID}}->{$genus}) {
			$clustcladestats->{$clusts->{$readID}}->{$genus}=1;
		} else {
			$clustcladestats->{$clusts->{$readID}}->{$genus}++;
		}
		...
		if (not $clustcladestats->{$clusts->{$readID}}->{$family}) {
			$clustcladestats->{$clusts->{$readID}}->{$family}=1;
		} else {
			$clustcladestats->{$clusts->{$readID}}->{$family}++;
		}
		...
		if (not $clustcladestats->{$clusts->{$readID}}->{$order}) {
			$clustcladestats->{$clusts->{$readID}}->{$order}=1;
		} else {
			$clustcladestats->{$clusts->{$readID}}->{$order}++;
		}
		...
		if (not $clustcladestats->{$clusts->{$readID}}->{$class}) {
			$clustcladestats->{$clusts->{$readID}}->{$class}=1;
		} else {
			$clustcladestats->{$clusts->{$readID}}->{$class}++;
		}
		...
		if (not $clustcladestats->{$clusts->{$readID}}->{$phylum}) {
			$clustcladestats->{$clusts->{$readID}}->{$phylum}=1;
		} else {
			$clustcladestats->{$clusts->{$readID}}->{$phylum}++;
		}
		...
		$clustspecstats->{$clusts->{$readID}}->{$spec}=$clustcladestats->{$clusts->{$readID}}->{$spec};
...
print OUT "\n===============================\n";
my $unclassreads=$totreads-$clustedreads;
print OUT "UNCLASSIFIED: $unclassreads reads.\n"; 
